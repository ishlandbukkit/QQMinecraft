...
public class MessageHandler {
	...
	public void processCommand(String message, JsonObject sender) {
		...
		senderObj.age = sender.get("age") != null ? sender.get("age").getAsInt() : null;
		senderObj.area = sender.get("area") != null ? sender.get("area").getAsString() : null;
		senderObj.card = sender.get("card") != null ? sender.get("card").getAsString() : null;
		senderObj.level = sender.get("level") != null ? sender.get("level").getAsString() : null;
		senderObj.nickname = sender.get("nickname") != null ? sender.get("nickname").getAsString() : null;
		senderObj.role = sender.get("role") != null ? sender.get("role").getAsString() : null;
		senderObj.sex = sender.get("sex") != null ? sender.get("sex").getAsString() : null;
		senderObj.title = sender.get("title") != null ? sender.get("title").getAsString() : null;
		senderObj.user_id = sender.get("user_id") != null ? sender.get("user_id").getAsLong() : null;
			...
			if (handler.commandName().equals(commandName)) {
				...
				try {
					...
					plugin.getLogger().log(Level.SEVERE,
							"Error while passing command event to " + handler.getClass().getName(), t);
					Launcher.msgHandler.send("An internal error occurred while passing command event to "
